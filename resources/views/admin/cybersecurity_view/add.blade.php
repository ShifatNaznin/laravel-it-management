...
<div class="row">
  <div class="col-md-12">
    <form method="post" action="{{route('cybersecurity_view_store')}}" enctype="multipart/form-data">
      @csrf
      <div class="card">
        <div class="card-header header-part">
          <div class="row">
            <div class="col-md-6 card_header_title">
              <h3><i class="fa fa-gg-circle"></i> Add Cyber Security View</h3>
            </div>
        ...
        <div class="card-body">
          ...
          <div class="form-group row custom_form{{$errors->has('cybersecurity_id')? ' has-error':''}}">
            <label class="col-sm-3 col-form-label">Application Name:</label>
            <div class="col-sm-7">
              <select class="col-sm-12 form-control" name="cybersecurity_id">
                <option value="">Select CyberSecurity Category</option>
                @php
                $cybersecurityCategory=App\Cybersecurity::where('status', 1)->OrderBy('id', 'ASC')->get();
                @endphp
                @foreach ($cybersecurityCategory as $category)
                <option value="{{$category->id}}">{{$category->heading}}</option>
                @endforeach
              </select>
              @if ($errors->has('cybersecurity_id'))
              <span class="invalid-feedback" role="alert">
                <strong>{{ $errors->first('cybersecurity_id') }}</strong>
              </span>
              @endif
            </div>
          </div>
