            ...
            <div class="card-header header-part">
                <div class="row">
                    <div class="col-md-6 card_header_title">
                        <h3><i class="fa fa-gg-circle"></i> All Order Details</h3>
                    </div>
                ...
                <table cellspacing="0" bordercolor="gray" id="table_with_data"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-break">{{$data->order_id}}</td>
                            <td class="text-break">{{$data->product_id}}</td>
                            <td class="text-break">{{$data->product_name}}</td>
                            <td class="text-break">{{$data->category_id}}</td>
                            <td class="text-break">{{$data->price}}</td>
                            <td class="text-break">{{$data->qty}}</td>

                            <td>
                                <div class="btn-group btn-group-sm btn-color-ceate">
                                    <a href="{{route('orderdetails_view',$data->id)}}"
...
<div class="modal fade" id="softDelModal" tabindex="-1" role="dialog" aria-labelledby="SoftDelModalHeading"
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <form method="post" action="{{route('orderdetails_delete')}}">
            @csrf
