            ...
            <div class="card-header header-part">
                <div class="row">
                    <div class="col-md-6 card_header_title">
                        <h3><i class="fa fa-gg-circle"></i> All Address</h3>
                    </div>
                    {{-- <div class="col-md-6 text-right card_header_btn">
                        <a href="{{route('address_add')}}" class="btn"><i class="fa fa-plus-circle"></i> Add
                            Address</a>
                    </div> --}}
                </div>
            </div>
                ...
                <table cellspacing="0" bordercolor="gray" id="table_with_data"
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-break">{!!Str::words($data->address,15)!!}</td>

                            <td class="text-break">{{$data->linkedin}}</td>
                            <td class="text-break">{{$data->facebook}}</td>
                            <td class="text-break">{{$data->twitter}}</td>

                            <td>
                                <div class="btn-group btn-group-sm btn-color-ceate">
                                    <a href="{{route('address_view',$data->id)}}"
                                        class="btn btn-info view-btn">View</a>
                                    <a href="{{route('address_edit',$data->id)}}"
...
<div class="modal fade" id="softDelModal" tabindex="-1" role="dialog" aria-labelledby="SoftDelModalName"
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <form method="post" action="{{route('address_delete')}}">
            @csrf
