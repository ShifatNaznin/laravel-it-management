            ...
            <div class="card-header header-part">
                <div class="row">
                    ...
                    <div class="col-md-6 text-right card_header_btn">
                        <a href="{{route('product_add')}}" class="btn"><i class="fa fa-plus-circle"></i> Add
                            Product</a>
                    </div>
                </div>
            </div>
            <div id="printableTable" class="card-body table-responsive">
                <table cellspacing="0" bordercolor="gray" id="table_with_data"
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-break">{{$data->prodCategory->name}}</td>
                            <td class="text-break">{{$data->prodSubCategory->name}}</td>
                            <td class="text-break">{{$data->prodUnit->name}}</td>
                            <td class="text-break">{{$data->name}}</td>
                            <td class="text-break">{{$data->price}}</td>
                            <td class="text-break">{{$data->code}}</td>
                            <td class="text-break">{!!$data->details!!}</td>
                            <td class="text-break">{!!$data->feature!!}</td>

                            <td><img src="{{ asset('uploads/images/'.$data->photo) }}" style="height: 50px;"
                                    alt="">
                            </td>
                            <td><img src="{{ asset('uploads/images/'.$data->photo_two) }}" style="height: 50px;"
                                    alt="">
                            </td>
                            <td><img src="{{ asset('uploads/images/'.$data->photo_three) }}" style="height: 50px;"
                                    alt="">
                            </td>
                            ...
                            <td>
                                <div class="btn-group btn-group-sm btn-color-ceate">
                                    <a href="{{route('product_view',$data->id)}}" class="btn btn-info view-btn">View</a>
