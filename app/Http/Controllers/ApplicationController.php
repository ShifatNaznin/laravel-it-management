...
class ApplicationController extends Controller
{
    public function index(){
        $value=Application::orderBy('id','ASC')->get();
        return view('admin.application.index',compact('value'));
    }
    ...
    public function edit(Request $request, $id) {
        $data=Application::find($id);
        return view('admin.application.update', compact('data'));
    }

    public function update(Request $request) {
        $add=Application::find($request->id);
        $add->heading=$request->heading;
        $add->link=$request->link;
        $add->sub_heading=$request->sub_heading;
        // $add->subscribe_price=$request->subscribe_price;
    ...
    public function delete() {
        $id=$_POST['modal_id'];
        $delete=Application::where('status',1)->where('id',$id)->delete();
        if($delete) {
            return back()->with('success', 'value');
        }
