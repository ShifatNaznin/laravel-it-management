...
class AppsController extends Controller
{
    public function index(){
        $value=Apps::orderBy('id','ASC')->get();
        return view('admin.apps.index',compact('value'));
    }
    ...
    public function edit(Request $request, $id) {
        $data=Apps::find($id);
        return view('admin.apps.update', compact('data'));
    }

    public function update(Request $request) {
        $add=Apps::find($request->id);
        $add->heading=$request->heading;
        ...
        if ($request->hasFile('photo')) {
            ...
            $file=$request->file('photo');
            $image_heading=str_replace(' ', '_', substr($request->apps_heading, 0, 5)) . '-'. uniqid() . '.'. $file->getClientOriginalExtension();
            $image=Image::make($file);
    ...
    public function delete() {
        $id=$_POST['modal_id'];
        $delete=Apps::where('status',1)->where('id',$id)->delete();
        if($delete) {
            return back()->with('success', 'value');
        }
